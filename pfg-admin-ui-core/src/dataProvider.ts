...
/** Creates the shared React-admin REST adapter used by PFG admin surfaces. */
export function createRestDataProvider(
  client: Pick<AdminApiClient, "adminApiUrl" | "adminRequest">,
): DataProvider {
  ...
  function resourceUrl(resource: string, id?: Identifier): string {
    const baseUrl = `${client.adminApiUrl}/${encodeURIComponent(resource)}`;
    return id === undefined
      ? baseUrl
      : `${baseUrl}/${encodeURIComponent(String(id))}`;
  }
  ...
  /** Retrieves a paginated resource collection from its admin endpoint. */
  async function getList<RecordType extends RaRecord>(
    resource: string,
    params: GetListParams,
  ): Promise<GetListResult<RecordType>> {
    const response = await client.adminRequest(
      `${resourceUrl(resource)}?${listQuery(params)}`,
  ...
  async function getOne<RecordType extends RaRecord>(
    ...
    const response = await client.adminRequest(resourceUrl(resource, params.id), {
      signal: params.signal,
    });
  ...
  async function getMany<RecordType extends RaRecord>(
    ...
    const response = await client.adminRequest(`${resourceUrl(resource)}?${query}`, {
      signal: params.signal,
    });
    return {
  ...
  >(
    ...
    const response = await client.adminRequest(resourceUrl(resource), {
      method: "POST",
  ...
  async function updateMany<RecordType extends RaRecord>(
    ...
    await Promise.all(
      params.ids.map((id) =>
        client.adminRequest(resourceUrl(resource, id), {
          method: "PUT",
          body: JSON.stringify(params.data),
        }),
      ),
  ...
  async function deleteMany<RecordType extends RaRecord>(
    ...
    await Promise.all(
      params.ids.map((id) =>
        client.adminRequest(resourceUrl(resource, id), { method: "DELETE" }),
      ),
    );
