...
type RuntimeConfigValueType = "boolean" | "integer" | "string" | "json";
...
/** Groups and renders the runtime catalog while delegating validation to the API. */
...
function ConfigurationRow({ record }: { record: RuntimeConfigRecord }) {
  ...
  function saveValue() {
    let value: string | number | boolean | unknown[] | Record<string, unknown>;
    try {
      value = valueFromDraft(record, draft);
    } catch {
      notify("Invalid JSON value", { type: "error" });
      return;
    }
  ...
  return (
    ...
    >
      <Box sx={{ minWidth: 0 }}>
        ...
        >
          <Typography sx={{ fontWeight: 750 }}>{record.metadata.label}</Typography>
          <Chip
            size="small"
          ...
          {record.metadata.requiredForSetup ? (
      ...
      <Stack spacing={1.25}>
        {record.metadata.valueType === "boolean" ? (
          ...
          <TextField
            ...
            multiline={record.metadata.valueType === "json"}
            minRows={record.metadata.valueType === "json" ? 6 : undefined}
            value={String(draft)}
            disabled={disabled}
...
function draftFromRecord(record: RuntimeConfigRecord): DraftValue {
  ...
  if (record.metadata.valueType === "json") {
    return JSON.stringify(record.value ?? record.metadata.defaultValue, null, 2);
  }
...
function valueFromDraft(
  ...
  if (record.metadata.valueType === "json") {
    const parsed = JSON.parse(String(draft)) as unknown;
    if (
      parsed === null ||
      (typeof parsed !== "object" && !Array.isArray(parsed))
    ) {
      throw new Error("Structured configuration must be a JSON object or array");
    }
