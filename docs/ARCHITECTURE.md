...
[5. Verify]  Apply patch locally
             → run detected build + test command when a supported build system exists
             → capture command, stdout/stderr tails, timeout state and failure reason
...
[6. PR]      git checkout -B pfg/{issue-id}-{base-sha}-{suffix}
...
[7. Report]  POST /issues/{id}/done to pfg-hub
             ...
             → details includes verification and pull-request diagnostics
```
...
| Build + tests must pass before PR when detected | Avoid noisy PRs while still allowing small repos with no supported build system; skipped verification is reported explicitly |
...
The agent has the 7-phase contribution pipeline: claim, analyze, gather context,
generate a patch, verify it, open a pull request, and report the result back to
the hub. Autonomous pipeline completion is now part of the MVP: patch retries
start from a clean base revision, verification reports include structured
diagnostics, and PR creation handles branch uniqueness, empty commits, fork
fallback and GitHub API failures predictably.
...
1. **Repository execution safety**
   ...
   - Add language-specific setup and test command detection for common stacks
     before relying on manual runner configuration.
